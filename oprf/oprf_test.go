...
func TestNegotiateWithNewKey(t *testing.T) {
	...
	// send to server
	finData, evalReqEncoded, err := BlindMessage("this is my secret")
	...
	// send back to client
	evaluatedElementBytesEncoded, err := EvaluateMessage(keyEncoded, evalReqEncoded)
	...
	finMsgEncoded, err := FinalizeMessage(finData, evaluatedElementBytesEncoded)
